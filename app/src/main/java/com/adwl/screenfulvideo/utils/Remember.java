...
import com.adwl.screenfulvideo.manager.UIThreadDispatcher;
...
public class Remember {
    ...
    public static boolean isInitialized() {
        return INSTANCE.mWasInitialized;
    }
    ...
    private <T> Remember saveAsync(final String key, final T value, final Callback callback) {
        ...
        getExecutor().execute(new Runnable() {
            @Override
            public void run() {
                final boolean success = saveToDisk(key, value);
...
//        }.execute();
    ...
    public static void clear(final Callback callback) {
        ...
        getInstance().getExecutor().execute(new Runnable() {
            @Override
            public void run() {
                synchronized (SHARED_PREFS_LOCK) {
